...
def _resize2target(data: np.ndarray, enabled: bool = False, order: int = 0,
    ...
    if data.ndim == 4:
        min_size = tuple(data.shape[0] + list(min_size)) 
...
def _get_input(cfg,
    ...
    for i in range(num_vols):
        ...
        if label is not None:
            ...
            label[i] = _rescale(label[i], cfg.DATASET.LABEL_SCALE, order=0) 
